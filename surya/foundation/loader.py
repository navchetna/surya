...
#         return processor
# foundation/loader.py (updated with IPEX frontend integration)
from typing import Optional
import torch
from transformers.utils import is_flash_attn_2_available
...
class FoundationModelLoader(ModelLoader):
    ...
    def model(
        ...
        if device is None:
            device = settings.TORCH_DEVICE_MODEL
        if dtype is None:
            # See [https://github.com/pytorch/pytorch/issues/118122](https://github.com/pytorch/pytorch/issues/118122) - T4 (device version 7.5) will return true since it supports
            # emulated bf16, but falls back to very slow kernels, especially for SDPA
            if torch.cuda.is_bf16_supported(including_emulation=False):
                dtype = settings.MODEL_DTYPE_BFLOAT
            else:
        ...
        # NEW: frontend IPEX optimization (no compile)
        model._ipex_enabled = False
        model._ipex_bf16 = False

        # Apply IPEX only on CPU
        on_cpu = str(device).startswith("cpu")
        if HAS_IPEX and on_cpu:
            try:
                # BF16 first, with weights_prepack=False for stability
                model = ipex.optimize(
                    model,
                ...
                try:
                    ...
                    # Skip IPEX if all fail; proceed as original
                    pass

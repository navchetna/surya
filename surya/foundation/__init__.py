...
# foundation/init.py (updated with IPEX frontend integration)
from __future__ import annotations
from dataclasses import dataclass
from typing import List, Optional, Tuple
from collections import deque
...
class FoundationPredictor(BasePredictor):
    ...
    def decode(
        ...
        with settings.INFERENCE_MODE():
            # NEW: conditional BF16 autocast for CPU
            ctx = (
                torch.cpu.amp.autocast(dtype=torch.bfloat16, enabled=True)
                if (getattr(self.model, "_ipex_enabled", False) and getattr(self.model, "_ipex_bf16", False))
        ...
        print("Decode time taken: ", time.time() - start_time)
        return new_input, processed_output
    ...
    def prefill(
        ...
        print("Prefill time taken: ", time.time() - start_time)
        return new_input, processed_outputs, idxs_to_merge
    ...
    def prediction_loop(
        ...
        self.kv_cache = None
        torch.cuda.empty_cache()
        print("Total prediction loop time taken: ", time.time() - start_time)
        return predicted_tokens, batch_bboxes, scores, topk_probs

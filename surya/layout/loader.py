...
#         return processor
# layout/loader.py (updated with IPEX frontend integration)
from typing import Optional
        ...
        if HAS_IPEX and on_cpu:
            try:
                ...
                try:
                    ...
                    # Skip if all fail; continue as original Surya
                    pass

        # Keep compile if desired, but no IPEX-specific changes here
        if settings.COMPILE_ALL or settings.COMPILE_LAYOUT:
            torch.set_float32_matmul_precision("high")
            torch._dynamo.config.cache_size_limit = 16
            torch._dynamo.config.suppress_errors = False

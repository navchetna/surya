...
# recognition/init.py (updated with IPEX frontend integration notes)
# NOTE: This file primarily calls into FoundationPredictor, which is already IPEX-optimized.
# No direct model calls here, so no additional autocast is needed; benefits propagate from foundation.
# If future changes add direct model calls, wrap them in autocast as in other predictors.

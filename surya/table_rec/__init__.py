...
#         return results
# table_rec/init.py (updated with IPEX frontend integration)
from copy import deepcopy
from itertools import chain
from typing import List
...
from PIL import Image
from tqdm import tqdm
import contextlib  # NEW: for autocast gating
...
class TableRecPredictor(BasePredictor):
    ...
    def inference_loop(
        ...
        with settings.INFERENCE_MODE():
            ...
            while token_count < max_tokens:
                is_prefill = token_count == 0
                # NEW: conditional BF16 autocast for decoder on CPU
                dec_ctx = (
    ...
    def batch_table_recognition(
        ...
        for i in tqdm(range(0, len(images), batch_size), desc="Recognizing tables", disable=self.disable_tqdm):
            ...
            # We only need to process each image once
            with settings.INFERENCE_MODE():
                # NEW: conditional BF16 autocast for encoder on CPU
                enc_ctx = (
                    torch.cpu.amp.autocast(dtype=torch.bfloat16, enabled=True)
                    if (getattr(self.model, "_ipex_enabled", False) and getattr(self.model, "_ipex_bf16", False))
                    else contextlib.nullcontext()
